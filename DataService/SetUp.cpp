...
void SetUp::read(const QJsonObject &json)
{
    ...
    if (json.contains("shadows") && json["shadows"].isBool())
        shadows = json["shadows"].toBool();
    if (json.contains("softShadows") && json["softShadows"].isBool())
...
void SetUp::write(QJsonObject &json) const
{
    ...
    json["softShadows"] = softShadows;
    json["boundingVolumes"] = boundingVolumes;
...
void SetUp::print(int indentation) const
{
    ...
    QTextStream(stdout) << indent << "shadows:\t" << shadows << "\n";
    QTextStream(stdout) << indent << "softShadows:\t" << softShadows << "\n";
    QTextStream(stdout) << indent << "boundingVolumes:\t" << boundingVolumes << "\n";
...
void SetUp::setSoftShadows(bool b) {
    softShadows = b;
}
void SetUp::setNHints(bool b) {
    nHints = b;
}

void SetUp::setBoundingVolumes(bool b) {
    boundingVolumes = b;
}

void SetUp::setMAXDEPTH(int depth) {
    MAXDEPTH = depth;
}
